FactoryBot.define do
  ...
  factory :common_response, class: OpenStruct do
    skip_create
    corelogicPropertyId { "#{Faker::Number.number(digits: 5)}:#{Faker::Number.number(digits: 7)}" }
    compositePropertyId { "#{Faker::Number.number(digits: 5)}:#{Faker::Number.number(digits: 7)}" }
    links { [{}] }
  end
  ...
  factory :property, class: Corelogic::Property do
    ...
    zip4 { Faker::Number.number(digits: 4).to_s }
    state { Faker::Address.state_abbr }
    latitude { Faker::Address.latitude }
    longitude { Faker::Address.longitude }
    fipsCode { Faker::Address.zip.to_s }
    parcelNumber { Faker::Number.number(digits: 9).to_s }
    parcelSequence { Faker::Number.number(digits: 1).to_s }
    carrierRoute { "C002" }
    links {
      [
        { "rel" => "self",
